...
const NavBar = () => {
  ...
  const handleLogout = () => {
    logOut()
      .then(() => {
        // Redirect to homepage and reload page
    ...
    <div className="bg-[#f8f8f8] sticky top-0 z-50">
      <div className="navbar w-11/12 mx-auto">
        <div className="flex-1">
          <Link to="/" className="text-2xl font-bold text-[#cf3520]">
            Btracker
          </Link>
        </div>
        ...
        <div className="flex-none lg:hidden">
          <div className="dropdown dropdown-end">
            ...
            <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52">
              {user ? (
                <>
                  <li><Link to="/dashboard" className="hover:bg-[#cf3520] hover:text-white rounded">Dashboard</Link></li>
                  <li><Link to="/myTasks" className="hover:bg-[#cf3520] hover:text-white rounded">My Tasks</Link></li>
                  <li><Link to="/completedTasks" className="hover:bg-[#cf3520] hover:text-white rounded">Completed Tasks</Link></li>
                  <li>
                    <button
        ...
        <div className="hidden lg:flex gap-4 items-center">
          {user ? (
            <>
              <Link to="/dashboard" className="btn btn-ghost hover:bg-[#cf3520] hover:text-white">Dashboard</Link>
              <Link to="/myTasks" className="btn btn-ghost hover:bg-[#cf3520] hover:text-white">My Tasks</Link>
              <Link to="/completedTasks" className="btn btn-ghost hover:bg-[#cf3520] hover:text-white">Completed</Link>
